...
class Rating(tk.Frame):
    ...
    def save_rating(self):
        try:
            ...
            message = tk.messagebox.showerror(title="Error occurred",message= "Email not found")
    
    def successRating(self):
        success_label = tk.Label(self, text="Successful Rating",
                                 background='#997C70', foreground="black",
                                 font=("Times New Roman", 15))
        success_label.grid(row=0,column=6,pady=15)
        ...
        labelImage.grid(row=1,column=6)
...
class Reservation(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        mb.menu.add_checkbutton ( label="Parking 10€/day", variable=self.P)
        mb.menu.add_checkbutton ( label="Breakfast 5€/day", variable=self.B)
        mb.menu.add_checkbutton ( label="Electricity supply 3€/day", variable=self.E)
    ...
    def info(self):
        self.info_label = tk.Label(self, text="1) each room can accommodate up to 4 people", font=("Times New Roman", 10),
                                      background='#D8C4B6').grid(row=14, column=6)

        self.info_label2 = tk.Label(self,
                                   text="2) rvs can accommodate up to 8 people\t",
                                   font=("Times New Roman", 10),
                                   background='#D8C4B6').grid(row=15, column=6)
        self.info_label3 = tk.Label(self,
                                    text="3) children underage deserve a discount\t",
                                    font=("Times New Roman", 10),
                                    background='#D8C4B6').grid(row=16, column=6)
    ...
    def enter_data(self):
        try:
            ...
            dwmatia=cursor.execute('Select kwd_katal,xwrhtikothta FROM DWMATIO ORDER BY xwrhtikothta').fetchall()
            rv=cursor.execute('Select kwd_katal,xwrhtikothta FROM RV ORDER BY xwrhtikothta').fetchall()
            tents=cursor.execute('Select kwd_katal,xwrhtikothta FROM XWROS_KATASKHNWSHS ORDER BY xwrhtikothta').fetchall()
            ...
            message = tk.messagebox.showerror(title="Error occurred",message= "Error occurred, wrong data input")
...
class DatabaseRecords(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for table in tables:
            ...
            update_button = ttk.Button(frame, text="Update Selected Record", command=lambda tree=tree, table=table: self.update_selected_record(tree,table))
            update_button.pack()
            ...
            delete_button.pack()
            confirm_button = ttk.Button(frame, text="Confirm update", command=lambda tree=tree, table=table: self.confirm_update(tree,table))
            confirm_button.pack()

        conn.commit()
        conn.close()

    def update_selected_record(self,tree,table):
        ...
        for i, value in enumerate(selected_data):
            label = ttk.Label(self.root, text=f"Column {i + 1}:").grid(row=1,column=i)          
    ...
    def confirm_update(self,tree,table):
        selected_item = tree.selection()
        updated_data = [field.get() for field in self.entry_fields]
        conn = sqlite3.connect('camping.db')
        cursor = conn.cursor()

        cursor.execute(f"SELECT * FROM {table} LIMIT 1")
        columns = [desc[0] for desc in cursor.description]
        pk_col = columns[0]  # Assume first column is the primary key
        pk_value = updated_data[0]  # First entry in `updated_data` is the primary key
        updated_values = updated_data[1:]  # Exclude primary key for SET clause

        # Prepare the UPDATE query
        set_clause = ", ".join(f"{col} = ?" for col in columns[1:])  # Skip primary key
        update_query = f"UPDATE {table} SET {set_clause} WHERE {pk_col} = ?"

        # Execute the UPDATE query
        cursor.execute(update_query, updated_values + [pk_value])  # Include pk_value for WHERE
        conn.commit()
        conn.close()
